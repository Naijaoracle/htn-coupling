...
"$python_bin" "$root/scripts/stage4b_diagnose.py" prepare
"$python_bin" - "$root" <<'PY'
import json, subprocess, sys
from pathlib import Path
root = Path(sys.argv[1])
manifest = json.loads((root / "results/stage4b/run_manifest.json").read_text())
for case in manifest["cases"]:
    result = root / "results/stage4b/runs" / case["case"]
    subprocess.run(["julia", f"--project={root}",
                    str(root / "scripts/run_stage4_openbf.jl"),
                    case["yaml"], str(result)], check=True)
PY
"$python_bin" "$root/scripts/stage4b_diagnose.py" analyse
