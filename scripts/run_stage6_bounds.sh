...
stage6_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
pulse_root="${PULSE_ROOT:-../pulse-physiology-engine}"
...
exec "$pulse_root/build/venv/bin/python" "$stage6_root/scripts/run_stage6_bounds.py" "$@"
